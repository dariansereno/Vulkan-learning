...
namespace engine
{
	...
	bool EngineDevice::checkValidationLayerSupport()
	{
		...
		for (const char *layerName : validationLayers)
		{
			...
			std::cout << "current layer searched : `" << layerName << "`: " << std::endl;
	...
	void EngineDevice::pickPhysicalDevice()
	{
		...
		std::pair<int, VkPhysicalDevice> max = std::make_pair(0, VK_NULL_HANDLE);
		for (const auto &candidate : candidates)
		{
			if ((max.second == VK_NULL_HANDLE && candidate.first > 0) || (max.second != VK_NULL_HANDLE && candidate.first > max.first))
				max = candidate;
		}
		_physicalDevice = max.second;
	...
	int EngineDevice::rateDeviceSuitability(VkPhysicalDevice device)
	{
		...
		score += indices.graphicsFamily * 50;
		return score;
	};
